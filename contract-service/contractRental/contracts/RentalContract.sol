...
contract RentalContract {
    ...
    function endContract(uint _contractId) public {
        ...
        if (contractInfo.status == RentalStatus.NotCreated) {
            ...
            if (contractInfo.depositAmount > 0) {
                ...
                require(
                    address(this).balance >= contractInfo.depositAmount,
                    'Contract balance insufficient for deposit refund'
                );
    ...
    ) public payable onlyRenter(_contractId) rentalNotEnded(_contractId) {
        Contract storage contractInfo = contracts[_contractId];
    ...
    function cancelContractByOwner(
        uint _contractId,
        bool notifyBefore30Days
    ) public payable onlyOwner(_contractId) rentalNotEnded(_contractId) {
        Contract storage contractInfo = contracts[_contractId];
    ...
    function terminateForNonPayment(uint _contractId) public onlyOwner(_contractId) {
        ...
        if (contractInfo.depositAmount > 0) {
            require(
                address(this).balance >= contractInfo.depositAmount,
                'Contract balance insufficient for deposit forfeit'
            );
    ...
    function getContractDetails(uint _contractId) public view returns (Contract memory) {
        ...
        require(
            ...
            'Only the contract owner or renter can view the contract details'
        );
    ...
    function getContractTransactions(uint _contractId) public view returns (Transaction[] memory) {
        ...
        require(
            msg.sender == contractInfo.owner || msg.sender == contractInfo.renter,
            'Only the contract owner or renter can view the transactions'
        );
